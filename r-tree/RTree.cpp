...
void RTree::search(const int a_min[2], const int a_max[2], Node* a_node, vector<vector<pair<int, int>>>& result) {
    bool is_internal = a_node->IsInternalNode();

    for (int i = 0; i < a_node->m_count; ++i) {
        Rect rect(a_min[0], a_min[1], a_max[0], a_max[1]);

        if (Overlap2(&rect, &a_node->m_branch[i].m_rect)) {

            if (is_internal) {
                search(a_min, a_max, a_node->m_branch[i].m_child, result);

            } else {
                result.push_back(a_node->m_branch[i].m_data);
            }
        }
    }
}
